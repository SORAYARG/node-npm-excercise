...
function fillArray(arr, value) {
  return _.fill(arr, value) // Use the fill() function from lodash to fill the array with the value
}
...
function flatArrayAndCalculateMean(arr) {
  return _.mean(_.flatten(arr)) // Use the mean() function from lodash to calculate the mean of the array
  // Use the flatten() function from lodash to flatten the array
}

function sortByNamesAndCapitalizeFirstLetter(arr) {
  return _.sortBy(arr, "name").map((item) => _.capitalize(item.name)) // Use the sortBy() function from lodash to sort the array by name
...
function transformToSnakeCaseAndAllCaps(arr) {
  return arr.map((item) => _.snakeCase(item).toUpperCase()) // Use the snakeCase() function from lodash to convert the string to snake_case
